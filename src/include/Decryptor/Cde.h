...
inline std::string aes_ecb_decrypt(const std::string& cipherText, const std::string& key)
{
	...
	if (padding < 1 || padding > 128) { throw std::runtime_error("AES填充错误"); }
	...
	{
		throw std::runtime_error("AES填充错误");
	}
	return plaintext;
}
